...
public class GitHubUpdateChecker {
    ...
    private static List<String> fetchDynamicApiUrls() {
        ...
        try {
            HttpURLConnection connection = (HttpURLConnection) new URL(DYNAMIC_API_SOURCE).openConnection();
            ...
            try (InputStreamReader reader = new InputStreamReader(connection.getInputStream())) {
                ...
                // 假设返回的内容是每行一个 API 地址
                String[] urls = response.toString().split("\n");
                for (String url : urls) {
                    dynamicApiUrls.add(url.trim() + "/repos/LengMC/Lengbanlist/releases/latest");
                }
            }
        } catch (Exception e) {
    ...
    private static String fetchVersionFromUrl(String url) throws Exception {
        ...
        for (int i = 0; i < retryCount; i++) {
            try {
                ...
                lastException = e;
                Lengbanlist.getInstance().getLogger().warning("请求失败，重试中... (" + (i + 1) + "/" + retryCount + ")");
    ...
    public static void checkUpdate() {
        try {
            ...
            if (isUpdateAvailable(localVersion)) {
                ...
                Lengbanlist.getInstance().getLogger().info("喵喵发现现在是最新版本！");
            }
        } catch (Exception e) {
